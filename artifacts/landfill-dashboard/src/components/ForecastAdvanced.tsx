  ...
  return (
    <div className="flex flex-wrap gap-2 items-center">
      ...
      <Badge variant="outline" className="gap-1 text-[11px] text-muted-foreground">
        {data.monthlyDataPoints} mois d'historique
      </Badge>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      {data.siteProjections.map((site, i) => {
        ...
        const trendLabel = trend > 1 ? `+${trend.toFixed(1)}t/mois` : trend < -0.5 ? `${trend.toFixed(1)}t/mois` : "stable";
          ...
          <Card key={site.siteId} className="border border-border/60">
            <CardContent className="p-4">
              ...
              <div className="flex items-center gap-3 mt-2 text-[10px]">
                <span className="text-muted-foreground">Rythme actuel : {fmt(site.monthlyRateMt, 1)} t/mois</span>
